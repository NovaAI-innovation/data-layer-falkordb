# data-layer-falkordb — project isolation directive

This file is injected into the Agent Zero system prompt when this project
is active. It captures the workspace contract for the falkordb submodule.

## Workspace

ACTIVE WORKSPACE: `/a0/usr/projects/data-layer/data-layer-falkordb`

The workspace owns the falkordb graph service: server install, graph
bootstrap, node/edge schema, and per-project Agent Zero metadata.
...
1. Read `README.md`, `docs/graph-schema.md`, and `AGENTS.md` before changing anything.
...
Cross-component wiring goes through the umbrella's `bootstrap falkordb`
